...
This is an **agent-based system** using five specialized AI agents that operate through file manipulation rather than traditional code execution:
...
- **回音 (Huí Yīn)** (`.claude/agents/回音.md`): Generates multi-dimensional productivity reports integrating tasks, emotions, and time data; maintains user preference profile
- **灵犀 (Líng Xī)** (`.claude/agents/灵犀.md`): Performs long-term pattern analysis and cognitive bias identification
- **明镜 (Míng Jìng)** (`.claude/agents/明镜.md`): Provides real-time cognitive bias checks and methodology guidance based on user preference profile
...
├── user_preference.md # User preference profile (maintained by 回音 Agent)
...
6. 回音 Agent generates comprehensive analysis and updates `data/user_preference.md`
7. 明镜 Agent reads user preference profile to provide personalized real-time reminders
...
- **明镜 Agent provides real-time cognitive bias checks during work sessions**
- Import pomodoro timer data at day end
- Generate reports using 回音 Agent agent (which also updates user preference profile)
...
- **回音 Agent**: Works only when user specifically asks for summaries (daily/weekly/monthly reports); also updates user preference profile
- **明镜 Agent**: Works during work sessions to provide real-time reminders and cognitive bias checks based on user preference profile
...
- **Cognitive Bias Awareness**: Identifies planning optimism/conservatism patterns; 明镜 Agent provides real-time bias checks
- **Personalized Reminders**: User preference profile enables increasingly accurate and personalized guidance
